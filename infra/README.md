...
Migrations (table creation) are applied automatically on every deployment by
`deploy-prod.sh`/`deploy-staging.sh` (`npx prisma migrate deploy`).

**One time only, to carry over existing data** from the old
`server/data/db.json` (products, meal deals, orders, templates):
```bash
sudo -u bde-app node prisma/import-from-json.mjs        # prod
sudo -u bde-app-staging node prisma/import-from-json.mjs # staging
```
Run this right after the very first deployment (once the tables are created),
never again afterwards — it duplicates data if run against an already-populated database.
